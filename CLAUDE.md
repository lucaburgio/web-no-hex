...
- **Artillery** may have **`range`**: it can either **move** or **shoot** at an enemy in one turn (not both). On a **standard hex board**, valid ranged targets are at **hex distance 2–`range`**. On **polygon territory maps**, any enemy whose territory center lies within **max-radius** of yours (same circle as the UI; centroid distance ÷ average neighbor spacing, **no minimum**; mountains do not block). **Melee-adjacent** enemies (same neighbor relation as movement) are **never** ranged targets—**move** to attack for simultaneous combat. Ranged hits deal no damage to the artillery and do not conquer the hex if the defender dies; **adjacent** hex-board combat still uses normal simultaneous damage and advance rules when you move to attack.
...
- **Polygon territory maps:** Movement and melee adjacency use `effectiveGetNeighbors()` → `customMapGraph.adjacency`. That graph is built by `buildTerritoryAdjacency()` from **shared polygon edges** (the same undirected edge must appear as consecutive vertices on both territory boundaries), aligned with `mapEditor.ts` / `buildEdgeTerritoryIndex` in `territoryRenderer.ts`. **`sanitizeTerritoryMapDef()`** (called from `buildTerritoryGraph()` and editor import/export) removes a redundant **outer** territory when the interior is fully tiled by smaller faces (any mix of neutral / enemy / allied / mountain), **their areas sum to the parent’s area**, and **every parent boundary edge appears on some child** — this catches the common mistake of splitting a region into new faces (auto-detect / “save as new”) **without deleting** the original enclosing territory (e.g. neutral `t1` left around a mixed interior). A stricter same-state-only subset rule still applies for smaller candidate sets. Optional **`adjacencyBlockPairs`** still removes selected edges after the shared-edge pass. On load, `applyGameStateBoardDimensions()` rebuilds the graph from embedded `mapDef`. Dropping territories **changes** neutral/mountain **virtual column** indices (assignment is by order within each state group); prefer fixing the JSON once rather than relying on saves across territory-count edits. **Artillery ranged range** on polygon maps uses straight-line centroid distance divided by `avgAdjacentCentroidPx` (same max-radius circle as the UI: valid targets satisfy 0 < distance ≤ `range` in those units, no minimum; **melee-adjacent** enemies excluded—move to attack; mountains do not block). Each **movement step** across one border still costs **1 MP** (same as hex boards).
