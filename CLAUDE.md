...
- **Breakthrough:** south = attacker (fixed PP pool, no further income); north = defender (normal PP + territory). Map split into **sectors**; the **starting southern sector has no CP** (already yours). Other sectors have **control points** until captured; attacker captures by holding CP for **two full rounds** (irreversible). On capture, CP marker is removed, **all hexes in that sector** become attacker territory, attacker gains **bonus PP** (`breakthroughSectorCaptureBonusPP`); the defender cannot regain those hexes. Northern units in attacker-held sectors fight at reduced CS. Settings include `breakthroughAttackerStartingPP`, `breakthroughSectorCount`, `breakthroughEnemySectorStrengthMult`, `breakthroughSectorCaptureBonusPP`.
