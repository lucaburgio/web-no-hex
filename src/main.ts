...
function buildRulesContent(): string {
  ...
  return `
    ...
    <section id="rules-game-modes" class="rules-anchor-section">
      ...
      <ul class="rules-list">
      <li><strong>Domination:</strong> <strong>conquer all territories</strong> (mountains excluded), or <strong>eliminate all enemy units</strong>.</li>
      <li><strong>Conquest:</strong> <strong>control points</strong> are placed on territories in the editor (or embedded map JSON).
        ...
        Conquering territories alone does <strong>not</strong> end the match — only Conquer Points determine the winner.
...
svg.addEventListener('click', (e: MouseEvent) => {
  ...
  if (offTurnInspect) {
    ...
    if (clickedUnit && clickedUnit.owner === enemyOwner) {
      vsHumanOffTurnInspectUnitId = eventTargetIsOnBoardUnitChip(e) ? clickedUnit.id : null;
    } else {
      vsHumanOffTurnInspectUnitId = null;
    }
    render();
